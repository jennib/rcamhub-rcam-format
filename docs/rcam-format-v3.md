...
```jsonc
{
  ...
  "features": [ /* re-editable generator output, see below */ ]
}
```
...
| `ruida` | Ruida DSP (`.rd`) binary format [EXPERIMENTAL] |

(Legacy laser files that stored `"grbl"` map to `grbl-dynamic`. Ruida DSP is an experimental binary format emitting scrambled `.rd` bytecode rather than text G-code.)
...
| `polyline` | `points` (Vec2[]), `vertexIds` (string[], optional), `closed` (bool), `cornerRadii` (object keyed by vertex id, optional), `cornerType` / `cornerTypes` (optional), `polygon` (optional, see below) | vertices `v<id>`; segment mids `mid_<id>` (id of the segment's start vertex) | `cr` corner size |
...
| `polybezier` | `nodes[]` (each `{id, pos, inH, outH, smooth, symmetric}`), `closed` (bool, optional), `fit` (bool, optional — true means the handles are COMPUTED so the curve passes through its knots; they are still written, so a reader ignoring the flag gets the same shape) | knots `n<id>` (solver DOFs, constrainable); handles `n<id>.in` / `n<id>.out` (draggable only — neither solver DOFs nor constraint targets, because a smooth node's handles move each other) | — |
| `point` | `pos` (Vec2) | `p` | — |
...
- A **regular polygon keeps a note saying so.** A polyline drawn with the Polygon
  tool carries `polygon: { sides, radius, rotation }` — `sides` is the side count
  (3–64), `radius` the CIRCUMradius (centre to a vertex, not across the flats),
  and `rotation` the angle in radians of vertex 0. It lets an editor offer "8
  sides, 40mm across flats" instead of sixteen loose numbers. It is dropped the
  moment a vertex is hand-edited, because the shape is then no longer regular and
  the note would be a lie. Omit it and you get an ordinary polyline, which is
  always safe.
...
nor to a feature's `keyIds`. Those are keyed by vertex id and by generator-emitted
entity name, so they scale with the drawing rather than with a fixed vocabulary.
`livingHinge` emits one key per slit and `spoilboard` one per hole, counterbore
and t-slot, and the params driving those counts declare a `min` and no `max` — a
2400 mm table at 10 mm spacing is some 28,800 holes. There is no ceiling to
derive, and one copied from the param maps would refuse real work.

`keyIds` is still bounded per ENTRY: keys at 64 characters and the entity ids
they point at at 128, which is safe at any count. The absence of a count limit
is asserted by a test, so that tidying it into line with the param maps fails
rather than quietly refusing a spoilboard nobody has drawn yet.
...
## Features (generator output)

`features[]` records geometry a built-in generator produced — a finger box, a
spur gear, a kumiko lattice — so it stays **re-editable** instead of becoming
anonymous lines the moment it is drawn. Change a parameter and the feature is
regenerated in place.

```jsonc
{
  "id": "feat-1",
  "generatorId": "box-joint",        // which generator made it
  "params": { "width": 120, "fingers": 5 },   // the re-run inputs, resolved values
  "paramExprs": { "width": "boxW" }, // optional: a formula per param
  "groupId": "grp-3",                // the group holding this feature's entities
  "offset": { "x": 40, "y": 20 },    // optional: where the output was placed
  "keyIds": { "front-wall": "ent-9" } // optional: generator key -> entity id
}
```

- **`groupId` is the link to the geometry.** A feature owns no entities directly;
  it names the group that holds them. That group must exist, and the usual rule
  applies — an entity belongs to at most one group.
- **`params` are the last resolved values**, and are the fallback. A param with a
  `paramExprs` entry is re-evaluated against `variables` and the built-in
  keywords (`stock`, `pi`, …) before every solve; one whose expression fails to
  evaluate keeps its cached number. So a file never needs an expression to
  evaluate in order to load.
- **`keyIds` survives reordering.** A generator names the entities it cares about
  (`"front-wall"`), and regeneration pairs them up by KEY rather than by emit
  position — so a constraint, dimension or toolpath pointing at generated
  geometry still points at the same part of it after a parameter edit. Entries
  whose entity has been deleted are pruned.
- Entity ids are otherwise **stable across a regeneration**, which is what lets
  CAM operations reference generated geometry at all.

Writing `features[]` by hand is not the usual path — emit the geometry and leave
the array out. It is documented because a file that carries one is asserting that
the geometry can be rebuilt from these numbers, and a wrong `groupId` or a
`generatorId` this build does not know makes that false.
...
Every one is generated from a builder in `scripts/` and checked byte-for-byte, so
what is described here is what the file contains. `test/guideListsEveryExample.test.ts`
fails if this list and the directory disagree — the list is hand-written, and it
had gone stale by eleven files before that check existed.

**Sketch — geometry, constraints, parameters**
...
- `bracket.rcam` — L-profile driven to "fully constrained" with per-segment H/V constraints.
  Worth reading for how FEW dimensions it needs: six vertices are twelve degrees of
  freedom, the H/V constraints take six and the corner pin two, so exactly four remain.
- `bolt-circle.rcam` — `variables` + a circular `pattern`.
- `parametric-box.rcam` — the three parametric channels side by side: a dimension
  `expr`, a `ScalarBinding` (which drives geometry and draws nothing), and `stock`
  as a built-in.
- `block-hole-pattern.rcam` — `blockDefinitions` + `groups` with `definitionId`: one
  definition placed four times, each instance `rigid` with its own `angle`. Note that
  a rigid block's degrees of freedom belong to the block, so a `fixedPoint` on one of
  its members is dropped as internal — `fixed` is what holds an instance.
- `rcam-logo.rcam` — the only file with no `operations` at all: `isConstruction`
  geometry, a `bezier`, and driving `diameter` dimensions.

**CAM — toolpaths and the order they run in**
...
- `clamped-toolplate.rcam` — a positioned `stockRect` (the blank inside a larger work
  area), a `features[]` entry from the `clamp` generator, a layer with `fixture: true`
  so pre-flight treats its shapes as keep-outs, and the only use of
  `pocketStrategy: "adaptive"`. Note `safeZ` clears the *fixtures*, not the stock.
- `nested-sheet.rcam` — many parts on one sheet: holes bored while each part is still
  attached, all outlines cut last, tabs on every part.
- `flip-spacer-block.rcam` — `flip` + per-operation `face`: two setups, two posted
  programs, located by registration pins whose positions must be unchanged by the
  mirror.
- `surfaced-sign-blank.rcam` — `face` (which names no geometry, taking its extent from
  the blank) followed by a profile carrying `startDepth`, because skimming does not
  move the Z datum.
- `box-joint-tab-slot.rcam` — `cornerStyle: "tbone"`, without which a round cutter
  leaves a fillet in every square inside corner and the joint will not seat.
- `vcarve-sign.rcam` — `vcarve` + `chamfer` operations sharing one v-bit via the `tools` library.
- `inlay-coaster.rcam` — `inlay`, where `glueGap`, `sawAllowance` and `pocketDepth` are
  a fit rather than three independent preferences.
- `relief-medallion.rcam` — a height-map `image` with `zRangeMM` (so the bytes are
  lengths, not tone), roughed by `relief-rough` then finished.

**Other machine kinds**

- `laser-coaster.rcam` — `machineKind: "laser"` with outline/fill engraves and a kerf-compensated cut.
- `fold-tray.rcam` — `score` beside `profile`, on layers carrying their own `laser`
  recipes, so re-tuning a layer moves every operation on it.
...
In both rotary files the canvas height is π × diameter **exactly**, because the
canvas *is* the unrolled surface: a rounded constant leaves the wrap seam open.
